...
def test_install_wallclock_timeout_fires(monkeypatch):
    ...
    # _kill() does interrupt_main(), sleeps 2s, then os._exit(124).
    # Sleep past the full drain so the mocked os._exit runs before
    # monkeypatch tears down — otherwise the real os._exit kills pytest.
    time.sleep(3.5)
