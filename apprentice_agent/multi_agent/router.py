...
class IntentRouter:
    ...
    def __init__(self, specialists: Dict[str, BaseSpecialist]):
        ...
        self._intent_cache: Dict[str, RoutingDecision] = {}
    ...
    def route(
        ...
        if multi_result:
            self._intent_cache[query_lower] = multi_result
            return multi_result
        ...
        if scores:
            ...
            if top_score >= 0.6:
                ...
                self._intent_cache[query_lower] = decision
                return decision

        # Step 4: Use LLM for ambiguous cases
        if llm_func and scores[0][1] < 0.4:
...
"""
        ...
        try:
            response = llm_func(system_prompt, f"Query: {query}")
