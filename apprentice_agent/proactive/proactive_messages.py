...
# Track recently sent messages to avoid repetition
_recent_messages: deque = deque(maxlen=20)


def _pick(messages: List[str]) -> Optional[str]:
    """Pick a message that hasn't been sent recently."""
    available = [m for m in messages if m not in _recent_messages]
    if not available:
        # All exhausted — clear history and pick fresh
        _recent_messages.clear()
        available = messages
    choice = random.choice(available)
    _recent_messages.append(choice)
    return choice
...
def get_emotional_message(
    ...
    if is_first_session:
        return _pick(_EMOTIONAL_FIRST_SESSION)
    if idle_hours > 1:
        return _pick(_EMOTIONAL_IDLE_LONG)
    if pleasure < -0.3:
        return _pick(_EMOTIONAL_LOW_PLEASURE)
    if pleasure > 0.3:
        return _pick(_EMOTIONAL_HIGH_PLEASURE)
    if arousal > 0.5:
        return _pick(_EMOTIONAL_HIGH_AROUSAL)
    return None
...
def get_social_message(idle_hours: float = 0) -> Optional[str]:
    """Get a social connection message based on idle duration."""
    if idle_hours > 1:
        return _pick(_SOCIAL_LONG_IDLE)
    elif idle_hours > 0.25:
        return _pick(_SOCIAL_MEDIUM_IDLE)
    return None
...
def get_competence_message(weak_areas: str = None) -> Optional[str]:
    """Get a competence/learning-driven message."""
    if weak_areas:
        msg = _pick(_COMPETENCE_WITH_AREAS)
        return msg.format(areas=weak_areas)
    return _pick(_COMPETENCE_GENERIC)
...
def get_coherence_message() -> str:
    """Get a coherence-driven message."""
    return _pick(_COHERENCE_MESSAGES)
...
def get_task_message(urgent: bool = False) -> str:
    """Get a task-related message."""
    if urgent:
        return _pick(_TASK_URGENT)
    return _pick(_TASK_OFFER)
