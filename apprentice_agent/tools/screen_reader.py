...
# Florence-2 singleton (lazy loaded, shared with vision.py)
_florence2_model = None
_florence2_processor = None
_florence2_available: Optional[bool] = None
...
def _check_florence2_available() -> bool:
    ...
    try:
        ...
        logger.info("[ScreenReader] Florence-2 unavailable (torch/transformers not installed)")
    return _florence2_available


def _load_florence2():
    """Lazy-load Florence-2 model for OCR."""
    ...
    logger.info("[ScreenReader] Loading Florence-2 from %s...", model_name)
    ...
    try:
        ...
        _florence2_model = None
        _florence2_processor = None
        raise RuntimeError(f"Florence-2 loading failed: {e}") from e

    logger.info("[ScreenReader] Florence-2 loaded on %s", device)
    return _florence2_model, _florence2_processor
