...
def _get_machine_key() -> bytes:
    ...
    # Use a combination of machine identifiers for key derivation
    machine_id_parts = []
    ...
    # username
    machine_id_parts.append(os.getenv("USERNAME", os.getenv("USER", "aura")))

    # Stable machine identifier
    machine_id_parts.append("aura-email-key-v1")

    seed = ":".join(machine_id_parts).encode("utf-8")
...
def _encrypt(plaintext: str) -> str:
    ...
    try:
        ...
        logger.warning("[Email] cryptography library not installed, storing credentials in plaintext")
        return plaintext
...
def _decrypt(ciphertext: str) -> str:
    ...
    try:
        ...
        return ciphertext  # If decryption fails, assume plaintext
...
class EmailTool:
        ...
        if EMAIL_CONFIG_FILE.exists():
            try:
                ...
                if config.get("app_password"):
                    config["app_password"] = _decrypt(config["app_password"])
                self._config = config
    ...
    def _save_config(self, config: dict):
        ...
        if save_config.get("app_password"):
            save_config["app_password"] = _encrypt(save_config["app_password"])
        try:
    ...
    def _connect_imap(self):
        ...
        if not self._is_configured():
            raise RuntimeError("Email not configured. Use 'setup' first.")
