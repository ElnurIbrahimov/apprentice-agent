...
class ProblemClassifier:
    ...
    def classify(self, query: str) -> ProblemCategory:
        ...
        best_category = ProblemCategory.ANALYSIS  # default fallback
        best_score = 0
            ...
            try:
                ...
                conn.execute(
                    ...
                    (
                        ...
                        json.dumps(all_metrics), time.time(),
                    ),
                )
                ...
                conn.execute(
                    """UPDATE strategy_outcomes
                       SET user_feedback = ?, metrics_json = ?,
                           composite_reward = ?
                       WHERE request_id = ?""",
                    (feedback, json.dumps(metrics), new_reward, request_id),
                )

                # Adjust arm parameters by delta
                conn.execute(
                    """UPDATE strategy_arms
                       SET alpha = alpha + ?,
                           beta = beta + ?,
