...
| apprentice_agent/aura_data/ | EXISTS | 15 files |
...
### Potential Conflicts
1. **Two Memory Systems:** Both `apprentice_agent/aura_data/` and `aura_data/` exist
   - Risk: Data fragmentation, unclear which is authoritative
   - Recommendation: Consolidate to single location

2. **Two AURA Implementations:** `apprentice_agent/` and `aura/` directories
   - `apprentice_agent/` - Full-featured monolithic agent
...
3. **Multiple Ollama Clients:**
   - `apprentice_agent/aura_ollama_client.py`
...
# Quick agent test
python -c "from apprentice_agent import ApprenticeAgent; a = ApprenticeAgent(); print('OK')"

# Life Modeling demo
python -c "from apprentice_agent import ApprenticeAgent; a = ApprenticeAgent(); print(a.life_simulate_decision('career_change', {'salary_change_pct': 0.2}))"
```
